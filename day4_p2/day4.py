...
def solve(paperMap: list[str]):
    ...
    while (new_solution != solution):
        ...
        result = countPapersAccessible(paperMap, heatmap, solution)
        new_solution = result[0]
        paperMap = result[1]

    return solution

def countPapersAccessible(paperMap: list(), heatmap: list(), solution: int):
    for line in range(len(heatmap)):
        for col in range(len(heatmap)):
            if heatmap[line][col] < 4 and paperMap[line][col] == PAPER_CHAR:
                mapLine = paperMap[line]
                paperMap[line] = mapLine[:col] + REMOVE_CHAR + mapLine[col + 1:]
                solution += 1
    
    return (solution, paperMap)
